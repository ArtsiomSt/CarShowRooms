...
def test_update_car(user_instance, status_code, get_token, car_brand, car, client, request):
    user_instance = request.getfixturevalue(user_instance)
    ...
    if status_code == 200:
        ...
        assert response.data == payload
        assert float(dealercar_instance.car_price) == round(car_price_dict['car_price'], 2)
        assert dealercar_instance.currency == car_price_dict['currency']
        payload_copy['car_brand_slug'] += 'brand_that_not_exists'
        response = client.put(
            endpoint, payload_copy, format="json", headers={"Authorization": "JWT " + auth_token}
        )
