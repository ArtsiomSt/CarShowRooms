...
def get_incomes_expenses():
    """This function creates report about all expenses and income in system"""
    ...
    showrooms["turnover"] = (
        SupplyHistory.objects.filter(details="Successful supply")
        .values("car_showroom_id", "car_showroom__name")
        .annotate(expense=Sum(F("car_price") * F("cars_amount")))
    )
    dealers["incomes"] = (
        SupplyHistory.objects.filter(details="Successful supply")
        .values("dealer")
        .annotate(income=Sum(F("car_price") * F("cars_amount")))
    )
    customers["expenses"] = TransactionHistory.objects.values(
        "made_by_customer"
    ).annotate(expense=Sum(F("deal_price")))
    showrooms_incomes = TransactionHistory.objects.values(
        "sold_by_showroom_id", "sold_by_showroom__name"
    ).annotate(income=Sum(F("deal_price")))
...
def get_cars_stats():
    """This function creates reports about car's supplies and purchases"""
    ...
    sold_cars_by_showrooms = TransactionHistory.objects.values(
        "sold_by_showroom_id", "sold_by_showroom__name", "car_id", "car__title"
    ).annotate(cars_sold=Count("car_id"))
    sold_cars_by_dealers = (
        SupplyHistory.objects.filter(details="Successful supply")
        .values("dealer_id", "dealer__name", "car_id", "car__title")
        .annotate(cars_sold=Count("car_id"))
    )
    purchased_cars_by_customers = TransactionHistory.objects.values(
        "made_by_customer_id", "made_by_customer__username", "car_id", "car__title"
    ).annotate(cars_bought=Count("car_id"))
    ...
    for showroom in showroom_objects:
        cars_by_showroom = {
            "car_showroom_id": showroom.pk,
            "car_showroom_name": showroom.name,
            "cars": [],
        }
        best_sold_car = {}
        for car in sold_cars_by_showrooms:
            if car["sold_by_showroom_id"] == showroom.pk:
                cars_by_showroom["cars"].append(
                    {
                        "car_id": car["car_id"],
                        "car_title": car["car__title"],
                        "cars_sold": car["cars_sold"],
                    }
                )
                if (
                    not best_sold_car
                    or best_sold_car.get("cars_sold", 0) < car["cars_sold"]
                ):
                    best_sold_car = {
                        "car_id": car["car_id"],
                        "car_title": car["car__title"],
    ...
    for customer in customer_objects:
        ...
        for car in purchased_cars_by_customers:
            if car["made_by_customer_id"] == customer.pk:
                cars_by_customer["cars"].append(
                    {
                        "car_id": car["car_id"],
                        "car_title": car["car__title"],
                        "cars_bought": car["cars_bought"],
                    }
                )
